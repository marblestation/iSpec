*
      subroutine CIAh2h2(omega,t,propac)
*
...
*
      ...
      real omega,t,propac,omegalast,qt,qw,h2h2op(maxtemp),x,exp10
...
*      save omegalast,iw,h2h2op,first,wave,temp,h2h2
...
*
      if (first) then
        open(33,file='DATA/CIA.H2-H2.Yi.dat', status='old')
        read(33,*) ident
        read(33,*) nwave
        read(33,*) ntemp
        if (ntemp.gt.maxtemp) stop 'CIAh2h2: maxtemp too small'
        if (nwave.gt.maxwave) stop 'CIAh2h2: maxwave too small'
...
* no extrapolation to higher wavenumbers!!
      ...
      if (omega.ne.omegalast) then
        ...
        do j=1,ntemp
          h2h2op(j)=qw*h2h2(j,iw+1)+(1.-qw)*h2h2(j,iw)
        enddo
      endif
      it=1
      do ii=1,ntemp-1

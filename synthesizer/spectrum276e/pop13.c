...
int N;
{
    ...
    if (approx(line[N].code, 6.0, 0.001) == 1) {
        for (i = 0; i < Ntau; i++)
            line[N].xnum[i] = model->NCI[i] * exp(-line[N].El / (k * model->T[i])) / pfunction(V, 6.0, i);
        return;
    }

    if (approx(line[N].code, 7.0, 0.001) == 1) {
        for (i = 0; i < Ntau; i++)
            line[N].xnum[i] = model->NNI[i] * exp(-line[N].El / (k * model->T[i])) / pfunction(V, 7.0, i);
        return;
    }

    if (approx(line[N].code, 8.0, 0.001) == 1) {
        for (i = 0; i < Ntau; i++)
            line[N].xnum[i] = model->NOI[i] * exp(-line[N].El / (k * model->T[i])) / pfunction(V, 8.0, i);
        return;
    }
    ...
    if (line[N].code < 100.0) {
        ...
        if (approx(line[N].code, cneutral, 0.001) == 1)
            charge = 0;
        else if (approx(line[N].code, c1ion, 0.001) == 1)
            charge = 1;
        else if (approx(line[N].code, c2ion, 0.001) == 1)
            charge = 2;
        else if (approx(line[N].code, c3ion, 0.001) == 1)
            charge = 3;
        else {
        ...
        for (i = 0; i < Ntau; i++) {
            Q4 = POP[l].Q4[i];
            if (charge == 0)
                line[N].xnum[i] = line[N].abund * model->NA[i] * exp(-1.1605e+4 * line[N].El / model->T[i]) / (Q4 * pfunction(V, cneutral, i));
            if (charge == 1)
                line[N].xnum[i] = line[N].abund * model->NA[i] * POP[l].R1[i] * exp(-1.1605e+4 * line[N].El / model->T[i]) / (Q4 * pfunction(V, c1ion, i));
            if (charge == 2)
                line[N].xnum[i] = line[N].abund * model->NA[i] * POP[l].R1[i] * POP[l].R2[i] * exp(-1.1605e+4 * line[N].El / model->T[i]) / (Q4 * pfunction(V, c2ion, i));
            if (charge == 3)
                line[N].xnum[i] = line[N].abund * model->NA[i] *
                    POP[l].R1[i] * POP[l].R2[i] * POP[l].R3[i] * exp(-1.1605e+4 * line[N].El / model->T[i]) / (Q4 * pfunction(V, c3ion, i));
        }
        return;
    } else {
        mu = line[N].chi2;
        D0 = line[N].chi1;
        /* Note: In isotope mode we are tacitly assuming that D0 is the same
           for all isotopic molecules.  This isn't correct, but is close
           enough.  As I find the data, I will add it in (Nov 30, 2004) */
        gffac = line[N].chi3;
        ...
        for (i = 0; i < Ntau; i++) {
            ...
            psip = 1.38054e-16 * model->T[i] * pow(10.0, D0 * theta - 13.670) * pow(theta, 2.5) / (pow(mu, 1.5)
                                                                                                   * Ua * Ub);
    ...
    for (i = 2; i < NATOM - NMOL; i++) {
        ...
        for (j = 0; j < Ntau; j++) {
            ...
            if (atom[i].maxcharge == 3) {
                R4 = (4.830e+15 / model->Ne[j]) * (pfunction(V, POP[i].atom + 0.4, j) / pfunction(V, POP[i].atom + 0.3, j)) * T15 * exp(-1.16e+4 * atom[i].I4 / model->T[j]);
                Q1 += R4;
            }
            if (atom[i].maxcharge >= 2) {
                POP[i].R3[j] =
                    (4.830e+15 / model->Ne[j]) * (pfunction(V, POP[i].atom + 0.3, j) / pfunction(V, POP[i].atom + 0.2, j)) * T15 * exp(-1.16e+4 * atom[i].I3 / model->T[j]);
                Q2 += POP[i].R3[j] * Q1;
            }
            if (atom[i].maxcharge >= 1) {
                POP[i].R2[j] =
                    (4.830e+15 / model->Ne[j]) * (pfunction(V, POP[i].atom + 0.2, j) / pfunction(V, POP[i].atom + 0.1, j)) * T15 * exp(-1.16e+4 * atom[i].I2 / model->T[j]);
                Q3 += POP[i].R2[j] * Q2;
                POP[i].R1[j] = (4.830e+15 / model->Ne[j]) * (pfunction(V, POP[i].atom + 0.1, j) / pfunction(V, POP[i].atom, j)) * T15 * exp(-1.16e+4 * atom[i].I1 / model->T[j]);
                POP[i].Q4[j] = 1.0 + POP[i].R1[j] * Q3;
            }
        }
        if (flagw == 1)
            printf("%2.0f  ", POP[i].atom);
    }
    if (flagw == 1)
        printf("\n");
}

...
int N;
{
    ...
    if (flagc == 0) {
        ...
        for (i = 1; i < Ntau; i++) {
            ...
            for (j = 0; j < N; j++) {
                if (line[j].ai == 1)
                    model->kapnu[i] += line[j].capnu[i] * autoion(wave, line[j].wave, line[j].gammar, line[j].gammas, line[j].gammaw);
                else {
                    v = 2.997929e+10 * (line[j].wave + delwave - wave) / (line[j].wave * line[j].dopp[i]);
    ...
    for (i = 1; i < Ntau; i++)
        y[i] = model->tauref[i] * model->kapnu[i] / (0.4343 * model->kapparef[i]);
    y[0] = 0;
    if (wave > 3645.0 && wave < 3750.0)
        opttrap(y, model->x, Ntau - 1, model->taunu);
    else

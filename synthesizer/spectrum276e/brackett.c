...
double wave;
{
    ...
    if (ifcore == 0 && flag == 0) {
        ...
        for (m = 5; m < 35; m++) {
            ...
            if (fabs(wave - wave0[m]) < rad) {
                for (i = 0; i < Ntau; i++) {
                    for (j = 0; j < 4; j++)
                        bkap2[j][i] += unified(4, m, i, 12.74607, Eh[m], model, lambda[j]);
                }
            }
        }
        flag = 1;
    ...
    if (ifcore == 0 && wave >= lambda[2]) {
        ...
        for (m = 5; m < 35; m++) {
            ...
            if (fabs(wave - wave0[m]) < rad) {
                for (i = 0; i < Ntau; i++)
                    bkap2[3][i] += unified(4, m, i, 12.74607, Eh[m], model, lambda[3]);
            }
        }
    }
        ...
        for (i = 0; i < Ntau; i++) {
            hkap[i] = bkap2[3][i] * ((wave - lambda[0]) *
                ...
                bkap2[1][i] * ((wave - lambda[0]) *
                               (wave - lambda[2]) * (wave - lambda[3])) / 2.0 - bkap2[2][i] * ((wave - lambda[0]) * (wave - lambda[1]) * (wave - lambda[3])) / 2.0;
        }
    }
    if (ifcore == 1) {
        for (i = 0; i < Ntau; i++)
        ...
        for (m = 5; m < 35; m++) {
            ...
            if (fabs(wave - wave0[m]) < rad) {
                for (i = 0; i < Ntau; i++)
                    hkap[i] += unified(4, m, i, 12.74607, Eh[m], model, wave);
            }
        }
        flag = 0;
    }

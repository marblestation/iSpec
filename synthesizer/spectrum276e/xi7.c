...
double code, T, Ne;
{
    ...
    if (approx(code, 101.0, 0.001) == 1) {
        ...
        Ub = partfn(7.0, T, Ne);
        mu = 0.94028;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(3.47 / (k * T))
            * pow(T, -1.5);
    }
        ...
        Ub = partfn(12.0, T, Ne);
        mu = 0.96780;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(1.34 / (k * T))
            * pow(T, -1.5);
    }
        ...
        Ub = partfn(13.0, T, Ne);
        mu = 0.97170;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(3.06 / (k * T))
            * pow(T, -1.5);
    }
        ...
        Ub = partfn(20.0, T, Ne);
        mu = 0.98321;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(1.70 / (k * T))
            * pow(T, -1.5);
    }
        ...
        Ub = partfn(6.0, T, Ne);
        mu = 6.0055;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(6.15 / (k * T))
            * pow(T, -1.5);
    }
        ...
        Ub = partfn(7.0, T, Ne);
        mu = 6.4627;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(7.66 / (k * T))
            * pow(T, -1.5);
    }
        ...
        xi = exp(9.763 / (k * T) - 4.8696e+01 + T * (1.9224e-03 + T * (-4.9143e-07 + T * (7.4692e-11 - T * 4.5399e-15))) - 1.5 * log(T));

    else if (approx(code, 708.0, 0.001) == 1)
        xi = exp(6.508 / (k * T) - 4.7365e+01 + T * (1.9840e-03 + T * (-4.9280e-07 + T * (7.2933e-11 - T * 4.3354e-15))) - 1.5 * log(T));
        ...
        xi = exp(5.116 / (k * T) - 4.8625e+01 + T * (1.6321e-03 + T * (-3.3915e-07 + T * (4.6025e-11 - T * 2.5839e-15))) - 1.5 * log(T));
        ...
        Ub = partfn(13.0, T, Ne);
        mu = 10.0436;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(5.27 / (k * T))
            * pow(T, -1.5);
    }
        ...
        Ub = partfn(14.0, T, Ne);
        mu = 10.1897;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(8.26 / (k * T))
            * pow(T, -1.5);
    }
        ...
        Ub = partfn(22.0, T, Ne);
        mu = 11.9921;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(6.87 / (k * T))
            * pow(T, -1.5);
    }
    ...
    /* VO 12.17542 */
        ...
        Ub = partfn(23.0, T, Ne);
        mu = 12.17542;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(6.41 / (k * T))
            * pow(T, -1.5);
    }
    ...
    /* YO 13.55929 */
        ...
        Ub = partfn(39.0, T, Ne);
        mu = 13.55929;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(7.29 / (k * T))
            * pow(T, -1.5);
    }
    ...
    /* ZrO 13.61204 */
        ...
        Ub = partfn(40.0, T, Ne);
        mu = 13.61204;
        xi = 5.322e-21 * (Uab / (Ua * Ub)) * pow(mu, -1.5) * exp(7.85 / (k * T))
            * pow(T, -1.5);
    }

    /* else if(approx(code,840.0,0.001) == 1) { logK =
       ply(11.5031,-1.1916,0.5692,-0.1795,0.0000,0.0000,logt)-7.8500*t; xi =
       1.0e+06*ksi*T/pow(10.0,logK); } */
...
double code, T;
{
    ...
    /* 
       if(approx(code,608.0,0.001) == 1) { Partition Function from Sauval &
       Tatum (1984) lgQ = ply(3.6076,-1.7608,0.4172,0.0,0.0,0.0,logt); } */
...
double h2opartfn(double t)
{
    static double qvib[40] = { 1.0000, 1.0032, 1.0227, 1.0633, 1.1239, 1.2036, 1.3028, 1.4186,
        1.5541,
        1.7085, 1.8825, 2.0767, 2.2919, 2.5289, 2.7886, 3.0721, 3.3804,
        3.7144,
        4.0752, 4.4637, 4.8807, 5.3272, 5.8038, 6.3112, 6.8499, 7.4203,
        8.0227,
        8.6572, 9.3238, 10.0225, 10.7531, 11.5153, 12.3085, 13.1325,
        13.9864,
        14.8698,
        15.7819, 16.7218, 17.6889, 18.6821
    };
    static double T[40] = { 200.0, 400.0, 600.0, 800.0, 1000.0, 1200.0, 1400.0, 1600.0,

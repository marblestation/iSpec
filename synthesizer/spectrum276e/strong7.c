...
double wave;
{
    static struct strong slines[] = {
        ...
        /*{2852.126, 12.0, 0, 0.0, 35051, 0.255, 100.0, 0.75, "01",  000.000, 0},*/
        ...
        {2585.876, 26.1, 0, 0, 38660, -0.190, 150.0, 1.00, "AO", 167.225,   0, 0},
        {2598.370, 26.1, 0, 385, 38859, -0.100, 150.0, 1.00, "AO", 167.222, 0, 0},
        {2599.396, 26.1, 0, 0, 38459, 0.350, 250.0, 1.00, "AO", 166.228,    0, 0},
        {2607.088, 26.1, 0, 668, 39013, -0.170, 150.0, 1.00, "AO", 167.224, 0, 0},
        {2611.874, 26.1, 0, 385, 38660, -0.050, 150.0, 1.00, "AO", 167.226, 0, 0},
        {2613.825, 26.1, 0, 863, 39109, -0.390, 150.0, 1.00, "AO", 167.223, 0, 0},
        {2617.618, 26.1, 0, 668, 38859, -0.570, 150.0, 1.00, "AO", 167.222, 0, 0},
        {2625.668, 26.1, 0, 385, 38459, -0.460, 150.0, 1.00, "AO", 166.230, 0, 0},
        {2628.294, 26.1, 0, 977, 39013, -0.450, 150.0, 1.00, "AO", 167.223, 0, 0},
        {2631.048, 26.1, 0, 863, 38859, -0.320, 150.0, 1.00, "AO", 167.222, 0, 0},
        {2631.324, 26.1, 0, 668, 38660, -0.300, 150.0, 1.00, "AO", 167.227, 0, 0},
        {2795.528, 12.1, 0, 0.0, 35761, 0.085, 500.0, 0.75, "AO",  246.357, 0, 0},
        {2802.705, 12.1, 0, 0.0, 35669, -0.218, 500.0, 0.75, "AO", 246.357, 0, 0},
        {2852.126, 12.0, 0, 0.0, 35051, 0.255, 100.0, 0.75, "01",  000.000, 0, 0},
        {3933.66, 20.1, 0, 0.0, 25414, 0.140, 400.0, 0.75, "AO",   234.223, 0, 0},
        {3968.47, 20.1, 0, 0.0, 25192, -0.162, 400.0, 0.75, "AO",  234.223, 0, 0},
        {4077.715, 38.1, 0, 0.0, 24517, 0.158, 15.0, 0.80, "AO",   262.208, 0, 0},

        {4226.730, 20.0, 0, 0.0, 23652, 0.243, 150.0, 1.00, "01",  000.000, 0},
    ...
    for (i = 0; i < NSTRG; i++) {
        if (fabs(wave - slines[i].wave) <= slines[i].radius) {
            if (slines[i].flag == 0) {
                ...
                if (approx(code, floor(code), 0.001) == 1)
                    icode = 0;
                else if (approx(code, floor(code) + 0.1, 0.001)
                         == 1)
                    icode = 1;
                else if (approx(code, floor(code) + 0.2, 0.001)
                         == 1)
                    icode = 2;
                else if (approx(code, floor(code) + 0.3, 0.001)
                         == 1)
                    icode = 3;
                else
                    icode = 0;
                ...
                if (icode == 0)
                    strgln[i].chi = atom[l].I1;
                else if (icode == 1)
                    strgln[i].chi = atom[l].I2;
                else if (icode == 2)
                    strgln[i].chi = atom[l].I3;
                else if (icode == 3)
                    strgln[i].chi = atom[l].I4;
                else
                    strgln[i].chi = atom[l].I2;
                ...
                for (j = 0; j < Ntau; j++)
                    strgln[i].xnum[j] = strgln[i].a[j] = strgln[i].dopp[j] = 0.0;
                pop(strgln, i, model, V, POP);
                broad(model, strgln, i, strgln[i].sig, strgln[i].alp, strgln[i].fac);
                capnu(strgln, i, model);
            }
            v = 2.997929e+10 * (strgln[i].wave - wave) / (strgln[i].wave * strgln[i].dopp[n]);
            /* The following factor involving dl takes into account the quasi- 
             */
            /* static contribution to van der Waal's broadening.  I have
               applied */
            /* it only to the red-wing, as this gives the best fit to the
               solar */
            /* Ca II K and H profiles, for reasons which I do not yet
               understand */
            ...
            dl = 0.0;
            kappa += strgln[i].capnu[n] * voigt((double)strgln[i].a[n], v) * (1.0 + dl / strgln[i].dlg[n]);
        }
    }

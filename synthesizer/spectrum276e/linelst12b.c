...
void getisotope();
void pop();
double initrad(linelist * list, int n, atominfo * atom, double loggf, double El, double Eu, double radmax, double teff, double logg, double code, double lambda);
double polishrad(linelist * list, int n, atominfo * atom, double rad, isodata * isotope, atmosphere * model, double Flux, pfunc * V, population * POP);
void broad(atmosphere * model, linedata * line, int N, double sig, double alp, double fac);
void capnu();
void eqtaukap();
double depth();

void linelst(wave, list, nlist, atom, teff, logg, qf, reset, isotope, model, Flux, V, POP, dwave)
...
FILE *qf;
{
    ...
    while (lastwave <= wave1) {
        ...
        /* If the transition type is AI (autoionizing), gammar = Gamma_shore,
           gammas = Ashore and gammaw = Bshore.  However, if Ashore, the
           asymmetry parameter, is negative, -log(|Ashore|), which will be a
           positive quantity, should be entered into the linelist file.  If
           Ashore is positive, the entry is log(Ashore), which is negative.
           The code below translates the various possibilities for GA and AI
           transition types */
        ...
        if (flagai == 1)
            ...
            rad = initrad(list, *nlist, atom, loggf, El, Eu, radmax, teff, logg, code, lambda);
...
double initrad(linelist * list, int n, atominfo * atom, double loggf, double El, double Eu, double radmax, double teff, double logg, double code, double lambda)
{
    ...
    if (list[n].code - floor(list[n].code) >= 0.01)
        lograd = 3.96 + (0.75861 + 0.0258773 * gfA) * gfA - 0.242223 * El;
    else
        lograd = 4.46 + (0.741125 + 0.0215996 * gfA) * gfA + ((0.0639498 * El - 0.311767) * El + 0.0891479) * El;
    if (lograd > log10(radmax))
        rad = radmax;
    else
        rad = pow(10.0, lograd);
    ...
    if (code - floor(code) < 0.05 && code < 100.0) {
        ...
        if (chi > Eu * 1.23981e-04) {
            ...
            if (neff > 10.0)
                rad *= pow(neff / 10.0, 2.0);
        }
    }
    /* Make adjustment for CNO atomic lines */
    if (code >= 6.0 && code < 9.0)
        rad *= 4.0;
    ...
    if (lambda > 8000.0 && lambda <= 13000.0)
        rad *= 2.7;
    if (lambda > 13000.0 && lambda < 19000.0)
        rad *= 3.8;
    if (lambda >= 19000.0 && lambda < 50000.0)
        rad *= 2.7;
...
double polishrad(linelist * list, int n, atominfo * atom, double rad, isodata * isotope, atmosphere * model, double Flux, pfunc * V, population * POP)
{
    ...
    if (approx(code, floor(code), 0.001) == 1)
        icode = 0;
    else if (approx(code, floor(code) + 0.1, 0.001) == 1)
        icode = 1;
    else if (approx(code, floor(code) + 0.2, 0.001) == 1)
        icode = 2;
    else if (approx(code, floor(code) + 0.3, 0.001) == 1)
        icode = 3;
    else
        icode = 0;
    i = NATOM + 1;
    ...
    if (flagI == 1 && code < 100.0) {
        relabund = 1.0;
        getisotope(isotope, code, oneline[0].iso, &oneline[0].atomass, &relabund);
    ...
    if (icode == 0)
        oneline[0].chi = atom[i].I1;
    else if (icode == 1)
        oneline[0].chi = atom[i].I2;
    else if (icode == 2)
        oneline[0].chi = atom[i].I3;
    else if (icode == 3)
        oneline[0].chi = atom[i].I4;
    else
        oneline[0].chi = atom[i].I2;
    oneline[0].abund = atom[i].abund;
    oneline[0].flag = 0;
    for (j = 0; j < Ntau; j++)
        oneline[0].xnum[j] = oneline[0].a[j] = oneline[0].dopp[j] = 0.0;
    ...
    while (fabs(10000.0 * (Depth - 0.0001)) > 0.20) {
        ...
        rad = (rad + radnew) / 2.0; /* Some lines will not converge without
                                       this */
        eqtaukap(wave + rad, model, oneline);
        Depth = depth(model, wave + rad, Flux);
        k++;

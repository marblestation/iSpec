...
int N;
{
    ...
    for (i = 0; i < N; i++) {
        ...
        if (i == 0) {
            c[0] =
                y[j + 1] / ((x[j + 1] - x[j]) * (x[j + 1] - x[j - 1])) - y[j] / ((x[j] - x[j - 1]) * (x[j + 1] - x[j])) + y[j - 1] / ((x[j] - x[j - 1]) * (x[j + 1] - x[j - 1]));
        ...
        c[1] = y[k + 1] / ((x[k + 1] - x[k]) * (x[k + 1] - x[k - 1])) - y[k] / ((x[k] - x[k - 1]) * (x[k + 1] - x[k])) + y[k - 1] / ((x[k] - x[k - 1]) * (x[k + 1] - x[k - 1]));
        ...
        if (c[1] == 0.0)
            ...
            w = fabs(c[1]) / (fabs(c[1]) + fabs(c[0]));
        am = w * a[0] + (1.0 - w) * a[1];
        bm = w * b[0] + (1.0 - w) * b[1];
        cm = w * c[0] + (1.0 - w) * c[1];
        if (c[1] != 0.0) {
            if (fabs(cm / c[1]) < 1.0e-14)
                cm = 0.0;
        }
        integral += (am + bm * (x[i + 1] + x[i]) / 2.0 + cm * (x[i + 1] * x[i + 1] + x[i + 1] * x[i] + x[i] * x[i]) / 3.0) * (x[i + 1] - x[i]);
        if (i >= 2)
            z[i + 1] = integral;
    }

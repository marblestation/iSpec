...
void free_dvector();
void printDensity();
/* Jan 19, 1997: Common metal hydrides, such as SiH, MgH, CaH, as well as TiO
   have been added to the equilibrium computations for these elements, but Si, 
   Mg and Ca have not yet been added to the equilibrium computations for CNO */
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        while (delta > 0.0001) {
            ...
            if (model->teff >= 6500.0)
                ...
                nmgI = amg * n[1] / (1.0 + pmg2 / n[7] + pmg2 * pmg3 / ne2 + EMgH * n[2] + EMgO * n[6]);
            if (model->teff >= 6500)
                nalI = aal * n[1] / (1.0 + pal2 / n[7] + pal2 * pal3 / ne2);
            else
                nalI = aal * n[1] / (1.0 + pal2 / n[7] + pal2 * pal3 / ne2 + EAlH * n[2] + EAlO * n[6]);
            if (model->teff >= 6500.0)
                nsiI = asi * n[1] / (1.0 + psi2 / n[7] + psi2 * psi3 / ne2);
            else
                nsiI = asi * n[1] / (1.0 + psi2 / n[7] + psi2 * psi3 / ne2 + ESiH * n[2] + ESiC * n[4] + ESiO * n[6]);
            nsI = as * n[1] / (1.0 + ps2 / n[7] + ps2 * ps3 / ne2);
            nkI = ak * n[1] / (1.0 + pk2 / n[7] + pk2 * pk3 / ne2);
            if (model->teff >= 6500.0)
                ncaI = aca * n[1] / (1.0 + pca2 / n[7] + pca2 * pca3 / ne2);
            else
                ncaI = aca * n[1] / (1.0 + pca2 / n[7] + pca2 * pca3 / ne2 + ECaH * n[2] + ECaO * n[6]);
            nfeI = afe * n[1] / (1.0 + pfe2 / n[7] + pfe2 * pfe3 / ne2);
            if (model->teff >= 6500.0)
                ntiI = ati * n[1] / (1.0 + pti2 / n[7] + pti2 * pti3 / ne2);
            else
                ntiI = ati * n[1] / (1.0 + pti2 / n[7] + pti2 * pti3 / ne2 + ETiN * n[5] + ETiO * n[6]);
            if (model->teff >= 6500.0)
                nzrI = azr * n[1] / (1.0 + pzr2 / n[7] + pzr2 * pzr3 / ne2);
            else
                nzrI = azr * n[1] / (1.0 + pzr2 / n[7] + pzr2 * pzr3 / ne2 + EZrO * n[6]);
            f[1][1] =
                n[2] + n[2] * ph / n[7] + n[2] * n[2] * ph2 +
                n[2] * n[2] * ph2p / n[7] + n[2] * n[7] * phm +
                n[3] + n[3] * phe2 / n[7] +
                n[3] * phe2 * phe3 / ne2 + n[4] +
                n[4] * pc2 / n[7] + n[4] * pc2 * pc3 / ne2 + n[5] +
                n[5] * pn2 / n[7] + n[5] * pn2 * pn3 / ne2 + n[6] +
                n[6] * po2 / n[7] + n[6] * po2 * po3 / ne2 +
                n[2] * n[4] * ECH + n[4] * n[4] * EC2 +
                n[4] * n[5] * ECN + n[4] * n[6] * ECO +
                n[5] * n[2] * ENH + n[5] * n[6] * ENO + n[5] * n[5] * EN2 + n[6] * n[2] * EOH + n[6] * n[6] * EO2 + n[2] * n[6] * n[2] * EH2O + n[7] - ntot;
            f[2][1] =
                n[2] + n[2] * ph / n[7] + 2.0 * n[2] * n[2] * ph2 +
                2.0 * n[2] * n[2] * ph2p / n[7] + n[2] * n[7] * phm + n[2] * n[4] * ECH + n[2] * n[5] * ENH + n[2] * n[6] * EOH + 2.0 * n[2] * n[6] * n[2] * EH2O - ah * n[1];
            f[3][1] = n[3] + n[3] * phe2 / n[7] + n[3] * phe2 * phe3 / ne2 - ahe * n[1];
            f[4][1] = n[4] + n[4] * pc2 / n[7] + n[4] * pc2 * pc3 / ne2 + n[4] * n[2] * ECH + 2.0 * n[4] * n[4] * EC2 + n[4] * n[5] * ECN + n[4] * n[6] * ECO - ac * n[1];
            f[5][1] = n[5] + n[5] * pn2 / n[7] + n[5] * pn2 * pn3 / ne2 + n[5] * n[2] * ENH + 2.0 * n[5] * n[5] * EN2 + n[4] * n[5] * ECN + n[5] * n[6] * ENO - an * n[1];
            f[6][1] =
                n[6] + n[6] * po2 / n[7] + n[6] * po2 * po3 / ne2 +
                n[6] * n[2] * EOH + 2.0 * n[6] * n[6] * EO2 + n[4] * n[6] * ECO + n[5] * n[6] * ENO + n[2] * n[6] * n[2] * EH2O - ao * n[1];
            f[7][1] =
                n[2] * ph / n[7] + n[2] * n[2] * ph2p / n[7] +
                n[3] * phe2 / n[7] +
                2.0 * n[3] * phe2 * phe3 / ne2 + n[4] * pc2 / n[7] +
                2.0 * n[4] * pc2 * pc3 / ne2 + n[5] * pn2 / n[7] +
                2.0 * n[5] * pn2 * pn3 / ne2 + n[6] * po2 / n[7] +
                2.0 * n[6] * po2 * po3 / ne2 + nnaI * pna2 / n[7] +
                ...
                nsiI * psi2 / n[7] +
                2.0 * nsiI * psi2 * psi3 / ne2 + nsI * ps2 / n[7] +
                2.0 * nsI * ps2 * ps3 / ne2 + nkI * pk2 / n[7] +
                2.0 * nkI * pk2 * pk3 / ne2 + ncaI * pca2 / n[7] + 2.0 * ncaI * pca2 * pca3 / ne2 + nfeI * pfe2 / n[7] + 2.0 * nfeI * pfe2 * pfe3 / ne2 - n[2] * n[7] * phm - n[7];

            df[1][1] = 0.0;
            df[1][2] = 1.0 + ph / n[7] + 2.0 * n[2] * ph2 + 2.0 * n[2] * ph2p / n[7] + n[7] * phm + n[4] * ECH + n[5] * ENH + n[6] * EOH + 2.0 * n[2] * n[6] * EH2O;
            df[1][3] = 1.0 + phe2 / n[7] + phe2 * phe3 / ne2;
            df[1][4] = 1.0 + pc2 / n[7] + pc2 * pc3 / ne2 + n[2] * ECH + 2.0 * n[4] * EC2 + n[5] * ECN + n[6] * ECO;
            df[1][5] = 1.0 + pn2 / n[7] + pn2 * pn3 / ne2 + n[4] * ECN + n[2] * ENH + n[6] * ENO + 2.0 * n[5] * EN2;
            df[1][6] = 1.0 + po2 / n[7] + po2 * po3 / ne2 + n[4] * ECO + n[5] * ENO + n[2] * EOH + 2.0 * n[6] * EO2 + n[2] * n[2] * EH2O;
            df[1][7] =
                -n[2] * ph / ne2 - n[2] * n[2] * ph2p / ne2 + n[2] * phm -
                n[3] * phe2 / ne2 - 2.0 * n[3] * phe2 * phe3 / ne3 -
                n[4] * pc2 / ne2 - 2.0 * n[4] * pc2 * pc3 / ne3 - n[5] * pn2 / ne2 - 2.0 * n[5] * pn2 * pn3 / ne3 - n[6] * po2 / ne2 - 2.0 * n[6] * po2 * po3 / ne3 + 1.0;
            df[2][1] = -ah;
            df[2][2] = 1.0 + ph / n[7] + 4.0 * n[2] * ph2 + 4.0 * n[2] * ph2p / n[7] + n[7] * phm + n[4] * ECH + n[5] * ENH + n[6] * EOH + 4.0 * n[2] * n[6] * EH2O;
            df[2][3] = 0.0;
            df[2][4] = n[2] * ECH;
            df[2][5] = n[2] * ENH;
            ...
            df[4][2] = n[4] * ECH;
            df[4][3] = 0.0;
            df[4][4] = 1.0 + pc2 / n[7] + pc2 * pc3 / ne2 + n[2] * ECH + 4.0 * n[4] * EC2 + n[5] * ECN + n[6] * ECO;
            df[4][5] = n[4] * ECN;
            df[4][6] = n[4] * ECO;
            ...
            df[5][3] = 0.0;
            df[5][4] = n[5] * ECN;
            df[5][5] = 1.0 + pn2 / n[7] + pn2 * pn3 / ne2 + n[2] * ENH + 4.0 * n[5] * EN2 + n[4] * ECN + n[6] * ENO;
            df[5][6] = n[5] * ENO;
            ...
            df[6][4] = n[6] * ECO;
            df[6][5] = n[6] * ENO;
            df[6][6] = 1.0 + po2 / n[7] + po2 * po3 / ne2 + n[2] * EOH + 4.0 * n[6] * EO2 + n[4] * ECO + n[5] * ENO + n[2] * n[2] * EH2O;
            ...
            df[7][7] =
                -n[2] * ph / ne2 - n[2] * n[2] * ph2p / ne2 -
                n[3] * phe2 / ne2 - 4.0 * n[3] * phe2 * phe3 / ne3 -
                ...
                ncaI * pca2 / ne2 - 4.0 * ncaI * pca2 * pca3 / ne3 - nfeI * pfe2 / ne2 - 4.0 * nfeI * pfe2 * pfe3 / ne3 - n[2] * phm - 1.0;
        ...
        model->rho[i] =
            1.66054e-24 * (1.008 * ah + 4.003 * ahe + 12.01 * ac +
                           14.01 * an + 16.00 * ao + 23.00 * ana + 24.32 * amg + 26.97 * aal + 28.06 * asi + 32.065 * as + 39.10 * ak + 40.08 * aca + 55.85 * afe) * model->NA[i];
...
atmosphere *model;
{
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        phe3 = 4.830e+15 * T32 * exp(-54.403 / kT) / partfn(2.1, T, Ne);
        fprintf(out,
                "%2d %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e\n",
                i + 1, model->Ne[i], model->NHI[i],
                model->NHI[i] * ph / model->Ne[i], model->NHminus[i],
                model->NH2[i], model->NHeI[i], model->NHeI[i] * phe2 / model->Ne[i], model->NHeI[i] * phe2 * phe3 / (model->Ne[i] * model->Ne[i]));
    }
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        po3 = 4.830e+15 * T32 * (partfn(8.2, T, Ne) / partfn(8.1, T, Ne)) * exp(-35.116 / kT);
        fprintf(out,
                "%2d %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e\n",
                i + 1, model->NCI[i], model->NCI[i] * pc2 / Ne,
                model->NCI[i] * pc2 * pc3 / ne2, model->NNI[i],
                model->NNI[i] * pn2 / Ne, model->NNI[i] * pn2 * pn3 / ne2, model->NOI[i], model->NOI[i] * po2 / Ne, model->NOI[i] * po2 * po3 / ne2);
    }
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        fprintf(out,
                ...
                model->NFeI[i] * pfe2 * pfe3 / ne2, model->NTiI[i], model->NTiI[i] * pti2 / Ne, model->NTiI[i] * pti2 * pti3 / ne2, model->NCaI[i], model->NCaII[i], model->N3[i]);
    }
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        fprintf(out,
                "%2d %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e\n",
                i + 1, C * H * ECH, N * H * ENH, O * H * EOH, C * C * EC2, C * N * ECN, C * O * ECO, N * N * EN2, N * O * ENO, O * O * EO2);
    }

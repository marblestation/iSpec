...
int ntau;
{
    ...
    if (flag == 0) {
        ...
        /* cutmin = cutmax = Cutoff[0]; for(i=1;i<Ntau;i++) { if(cutmin >
           Cutoff[i]) cutmin = Cutoff[i]; if(cutmax < Cutoff[i]) cutmax =
           Cutoff[i]; } cut = (cutmax+cutmin)/2.0; */
        flag = 1;
    }
    ...
    for (n = 1; n < 9; n++)
        nh[n] = model->NHI[ntau] * (2.0 * n * n) * exp(-13.595 * (1.0 - 1.0 / (n * n)) / (k * T)) / partfn(1.0, T, Ne);

    khbf = 0.0;
    for (n = 1; n < 9; n++)
        khbf += nh[n] * AN(n, nu, Cutoff[ntau], pCutoff[ntau], lCutoff[ntau]);
    khbf *= stim;
    khbf +=
        (2.815e+29 * model->NHI[ntau] / (pow(nu, 3.0) * partfn(1.0, T, Ne))) * stim * (k * T / 13.595) * (exp(-13.595 * (1.0 - 1.0 / 81.0) / (k * T)) - exp(-13.595 / (k * T)));
    ...
    khray = model->N1[ntau] * (5.799e-13 / pow(lam, 4.0) + 1.422e-6 / pow(lam, 6.0) + 2.784 / pow(lam, 8.0));

    /* Calculate H2 Rayleigh Scattering */

    lam = c / (double)dmin(nu, 2.922e+15);
    kh2ray = model->NH2[ntau] * (8.14e-13 / pow(lam, 4.0) + 1.28e-06 / pow(lam, 6.0) + 1.61 / pow(lam, 8.0));
    ...
    for (n = 1; n < 10; n++)
        nheii[n] = model->NHeII[ntau] * (2.0 * (double)(n * n)) * exp(-54.403 * (1.0 - 1.0 / (n * n)) / (k * T)) / partfn(2.1, T, Ne);

    kheIIbf = 0.0;
    for (n = 1; n < 10; n++)
        kheIIbf += nheii[n] * AHeIIN(n, nu);
    kheIIbf *= stim;
    kheIIbf +=
        (4.504e+30 * model->NHeII[ntau] / (pow(nu, 3.0) * partfn(2.1, T, Ne))) * stim * (k * T / 54.403) * (exp(-54.403 * (1.0 - 1.0 / 100.0) / (k * T)) - exp(-54.403 / (k * T)));
    ...
    lam2 = lam * lam;
    kheray = 5.484e-14 / lam2 / lam2 * pow(1.0 + (2.44e+5 + 5.94e+10 / (lam2 - 2.90e+5)) / lam2, 2.0);
    kheray *= NHeI1;
    ...
    if (flagO == 1)
        fprintf(opout,
                ...
                khbf, khff, khmbf2, khmff2, kh2, khray, khebf, kheIff, kheIIff, kheray, khemff, kh2ray, kheIIbf, klotemp, kluke, sig);

    return (khbf + khff + khmbf2 + khmff2 + kh2 + khray + khebf + kheIff + kheIIff + kheray + khemff + kh2ray + kheIIbf + klotemp + kluke + sig);
}
...
double nu, cutoff, pcutoff, lcutoff;
{
    ...
    if (n == 6 && wav > 33874.0)
        return (0.0);
    if (n >= 7 && nu < 3.28805e+15 / ((double)(n * n)))
        return (0.0);
...
double nu, T, Z;
{
    ...
    static float x2n[] = { -4.0, -3.5, -3.0, -2.5, -2.0, -1.5, -1.0, -0.5, 0.0, 0.5, 1.0,
        1.5
    };
    static float yb[11][12] = { {5.53, 4.91, 4.29, 3.64, 3.00, 2.41, 1.87, 1.33, 0.90, 0.55, 0.33,
                                 0.19},
    {5.49, 4.87, 4.25, 3.61, 2.98, 2.41, 1.89, 1.39, 0.95, 0.58, 0.36,
     0.21},
    {5.46, 4.84, 4.22, 3.59, 2.97, 2.41, 1.91, 1.44, 1.00, 0.62, 0.39,
     0.24},
    {5.43, 4.80, 4.18, 3.56, 2.95, 2.41, 1.93, 1.49, 1.08, 0.70, 0.46,
     0.28},
    {5.40, 4.77, 4.15, 3.54, 2.94, 2.41, 1.95, 1.55, 1.17, 0.85, 0.59,
     0.38},
    {5.25, 4.63, 4.02, 3.41, 2.81, 2.32, 1.90, 1.56, 1.30, 1.01, 0.76,
     0.53},
    {5.00, 4.40, 3.80, 3.22, 2.65, 2.19, 1.80, 1.51, 1.32, 1.15, 0.97,
     0.76},
    {4.69, 4.13, 3.57, 2.97, 2.44, 2.02, 1.68, 1.42, 1.30, 1.18, 1.09,
     0.96},
    {4.48, 3.87, 3.27, 2.70, 2.21, 1.84, 1.52, 1.33, 1.20, 1.15, 1.13,
     1.08},
    {4.16, 3.52, 2.98, 2.45, 2.01, 1.67, 1.41, 1.25, 1.15, 1.11, 1.10,
     1.09},
...
/* double AHminus(nu) double nu; { double anu;

   anu = 0.0; if(nu >= 2.111e+14) anu = 6.801e-20 + (5.358e-3 + (1.481e+13 +
   (-5.519e+27 + 4.808e+41/nu)/nu)/nu)/nu; if(nu < 2.111e+14 && nu >=
   1.8259e+14) anu = 3.695e-16 + (-1.251e-1 + 1.052e+13/nu)/nu; return(anu); } 
 */
...
double nu, T;
{
    ...
    Es = -7.342e-3 + (-2.409e-15 + (1.028e-30 + (-4.230e-46 + (1.244e-61 - 1.351e-77 * nu) * nu) * nu) * nu) * nu;
    F = exp(-Es / (k * T) - 3.0233e+03 + (3.7797e+02 + (-1.82496e+01 + (3.9207e-01 - 3.1672e-03 * log(nu)) * log(nu)) * log(nu)) * log(nu));
    return (F);
}
...
double lambda;
{
    /* The following polynomial fits Wishart's (1979) data to better than 0.2
       percent, see Gray, D.F. Obs and Analy of Stellar Phot */
...
double T, lambda;
{
    /* The following expression is a fit to the data of Bell and Berrington
       1987 */

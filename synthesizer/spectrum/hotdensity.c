...
void hotDensity(atmosphere * model, atominfo * atom, double ah, double ahe, int flagw)
{
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        while (delta > 0.0001) {
            ...
            ne4 = ne2 * ne2;
            ne5 = ne3 * ne2;
            nnaI = ana * n[1] / (1.0 + pna2 / n[7] + pna2 * pna3 / ne2 + pna2 * pna3 * pna4 / ne3 + pna2 * pna3 * pna4 * pna5 / ne4);
            nmgI = amg * n[1] / (1.0 + pmg2 / n[7] + pmg2 * pmg3 / ne2 + pmg2 * pmg3 * pmg4 / ne3 + pmg2 * pmg3 * pmg4 * pmg5 / ne4);
            nalI = aal * n[1] / (1.0 + pal2 / n[7] + pal2 * pal3 / ne2 + pal2 * pal3 * pal4 / ne3 + pal2 * pal3 * pal4 * pal5 / ne4);
            nsiI = asi * n[1] / (1.0 + psi2 / n[7] + psi2 * psi3 / ne2 + psi2 * psi3 * psi4 / ne3 + psi2 * psi3 * psi4 * psi5 / ne4);
            nsI = as * n[1] / (1.0 + ps2 / n[7] + ps2 * ps3 / ne2 + ps2 * ps3 * ps4 / ne3 + ps2 * ps3 * ps4 * ps5 / ne4);
            nkI = ak * n[1] / (1.0 + pk2 / n[7] + pk2 * pk3 / ne2 + pk2 * pk3 * pk4 / ne3 + pk2 * pk3 * pk4 * pk5 / ne4);
            ncaI = aca * n[1] / (1.0 + pca2 / n[7] + pca2 * pca3 / ne2 + pca2 * pca3 * pca4 / ne3 + pca2 * pca3 * pca4 * pca5 / ne4);
            nfeI = afe * n[1] / (1.0 + pfe2 / n[7] + pfe2 * pfe3 / ne2 + pfe2 * pfe3 * pfe4 / ne3 + pfe2 * pfe3 * pfe4 * pfe5 / ne4);
            ntiI = ati * n[1] / (1.0 + pti2 / n[7] + pti2 * pti3 / ne2);
            f[1][1] =
                n[2] + n[2] * ph / n[7] + n[2] * n[7] * phm + n[3] +
                n[3] * phe2 / n[7] + n[3] * phe2 * phe3 / ne2 +
                n[4] + n[4] * pc2 / n[7] + n[4] * pc2 * pc3 / ne2 +
                n[4] * pc2 * pc3 * pc4 / ne3 +
                n[4] * pc2 * pc3 * pc4 * pc5 / ne4 + n[5] +
                n[5] * pn2 / n[7] + n[5] * pn2 * pn3 / ne2 +
                n[5] * pn2 * pn3 * pn4 / ne3 +
                n[5] * pn2 * pn3 * pn4 * pn5 / ne4 + n[6] +
                n[6] * po2 / n[7] + n[6] * po2 * po3 / ne2 + n[6] * po2 * po3 * po4 / ne3 + n[6] * po2 * po3 * po4 * po5 / ne4 + n[7] - ntot;
            f[2][1] = n[2] + n[2] * ph / n[7] + n[2] * n[7] * phm - ah * n[1];
            f[3][1] = n[3] + n[3] * phe2 / n[7] + n[3] * phe2 * phe3 / ne2 - ahe * n[1];
            f[4][1] = n[4] + n[4] * pc2 / n[7] + n[4] * pc2 * pc3 / ne2 + n[4] * pc2 * pc3 * pc4 / ne3 + n[4] * pc2 * pc3 * pc4 * pc5 / ne4 - ac * n[1];
            f[5][1] = n[5] + n[5] * pn2 / n[7] + n[5] * pn2 * pn3 / ne2 + n[5] * pn2 * pn3 * pn4 / ne3 + n[5] * pn2 * pn3 * pn4 * pn5 / ne4 - an * n[1];
            f[6][1] = n[6] + n[6] * po2 / n[7] + n[6] * po2 * po3 / ne2 + n[6] * po2 * po3 * po4 / ne3 + n[6] * po2 * po3 * po4 * po5 / ne4 - ao * n[1];
            f[7][1] =
                n[2] * ph / n[7] + n[3] * phe2 / n[7] +
                2.0 * n[3] * phe2 * phe3 / ne2 + n[4] * pc2 / n[7] +
                2.0 * n[4] * pc2 * pc3 / ne2 +
                3.0 * n[4] * pc2 * pc3 * pc4 / ne3 +
                4.0 * n[4] * pc2 * pc3 * pc4 * pc5 / ne4 +
                n[5] * pn2 / n[7] + 2.0 * n[5] * pn2 * pn3 / ne2 +
                3.0 * n[5] * pn2 * pn3 * pn4 / ne3 +
                4.0 * n[5] * pn2 * pn3 * pn4 * pn5 / ne4 +
                n[6] * po2 / n[7] + 2.0 * n[6] * po2 * po3 / ne2 +
                3.0 * n[6] * po2 * po3 * po4 / ne3 +
                4.0 * n[6] * po2 * po3 * po4 * po5 / ne4 +
                nnaI * pna2 / n[7] +
                2.0 * nnaI * pna2 * pna3 / ne2 +
                3.0 * nnaI * pna2 * pna3 * pna4 / ne3 +
                4.0 * nnaI * pna2 * pna3 * pna4 * pna5 / ne4 +
                nmgI * pmg2 / n[7] +
                2.0 * nmgI * pmg2 * pmg3 / ne2 +
                3.0 * nmgI * pmg2 * pmg3 * pmg4 / ne3 +
                4.0 * nmgI * pmg2 * pmg3 * pmg4 * pmg5 / ne4 +
                nalI * pal2 / n[7] +
                2.0 * nalI * pal2 * pal3 / ne2 +
                3.0 * nalI * pal2 * pal3 * pal4 / ne3 +
                4.0 * nalI * pal2 * pal3 * pal4 * pal5 / ne4 +
                nsiI * psi2 / n[7] +
                2.0 * nsiI * psi2 * psi3 / ne2 +
                3.0 * nsiI * psi2 * psi3 * psi4 / ne3 +
                4.0 * nsiI * psi2 * psi3 * psi4 * psi5 / ne4 +
                nsI * ps2 / n[7] + 2.0 * nsI * ps2 * ps3 / ne2 +
                3.0 * nsI * ps2 * ps3 * ps4 / ne3 +
                4.0 * nsI * ps2 * ps3 * ps4 * ps5 / ne4 +
                nkI * pk2 / n[7] + 2.0 * nkI * pk2 * pk3 / ne2 +
                3.0 * nkI * pk2 * pk3 * pk4 / ne3 +
                4.0 * nkI * pk2 * pk3 * pk4 * pk5 / ne4 +
                ncaI * pca2 / n[7] +
                2.0 * ncaI * pca2 * pca3 / ne2 +
                3.0 * ncaI * pca2 * pca3 * pca4 / ne3 +
                4.0 * ncaI * pca2 * pca3 * pca4 * pca5 / ne4 +
                nfeI * pfe2 / n[7] +
                2.0 * nfeI * pfe2 * pfe3 / ne2 + 3.0 * nfeI * pfe2 * pfe3 * pfe4 / ne3 + 4.0 * nfeI * pfe2 * pfe3 * pfe4 * pfe5 / ne4 - n[2] * n[7] * phm - n[7];
            ...
            df[1][4] = 1.0 + pc2 / n[7] + pc2 * pc3 / ne2 + pc2 * pc3 * pc4 / ne3 + pc2 * pc3 * pc4 * pc5 / ne4;
            df[1][5] = 1.0 + pn2 / n[7] + pn2 * pn3 / ne2 + pn2 * pn3 * pn4 / ne3 + pn2 * pn3 * pn4 * pn5 / ne4;
            df[1][6] = 1.0 + po2 / n[7] + po2 * po3 / ne2 + po2 * po3 * po4 / ne3 + po2 * po3 * po4 * po5 / ne4;
            df[1][7] =
                -n[2] * ph / ne2 + n[2] * phm - n[3] * phe2 / ne2 -
                2.0 * n[3] * phe2 * phe3 / ne3 - n[4] * pc2 / ne2 -
                2.0 * n[4] * pc2 * pc3 / ne3 -
                3.0 * n[4] * pc2 * pc3 * pc4 / ne4 -
                4.0 * pc2 * pc3 * pc4 * pc5 / ne5 -
                n[5] * pn2 / ne2 - 2.0 * n[5] * pn2 * pn3 / ne3 -
                3.0 * pn2 * pn3 * pn4 / ne4 -
                4.0 * pn2 * pn3 * pn4 * pn5 / ne5 - n[6] * po2 / ne2 - 2.0 * n[6] * po2 * po3 / ne3 - n[6] * po2 * po3 * po4 / ne4 - n[6] * po2 * po3 * po4 * po5 / ne5 + 1.0;
            df[2][1] = -ah;
            ...
            df[4][2] = 0.0;
            df[4][3] = 0.0;
            df[4][4] = 1.0 + pc2 / n[7] + pc2 * pc3 / ne2 + pc2 * pc3 * pc4 / ne3 + pc2 * pc3 * pc4 * pc5 / ne4;
            df[4][5] = 0.0;
            df[4][6] = 0.0;
            df[4][7] = -n[4] * pc2 / ne2 - 2.0 * n[4] * pc2 * pc3 / ne3 - 3.0 * n[4] * pc2 * pc3 * pc4 / ne4 - 4.0 * n[4] * pc2 * pc3 * pc4 * pc5 / ne5;
            df[5][1] = -an;
            df[5][2] = 0.0;
            df[5][3] = 0.0;
            df[5][4] = 0.0;
            df[5][5] = 1.0 + pn2 / n[7] + pn2 * pn3 / ne2 + pn2 * pn3 * pn4 / ne3 + pn2 * pn3 * pn4 * pn5 / ne4;
            df[5][6] = 0.0;
            df[5][7] = -n[5] * pn2 / ne2 - 2.0 * n[5] * pn2 * pn3 / ne3 - 3.0 * n[5] * pn2 * pn3 * pn4 / ne4 - 4.0 * n[5] * pn2 * pn3 * pn4 * pn5 / ne5;
            df[6][1] = -ao;
            df[6][2] = 0.0;
            df[6][3] = 0.0;
            df[6][4] = 0.0;
            df[6][5] = 0.0;
            df[6][6] = 1.0 + po2 / n[7] + po2 * po3 / ne2 + po2 * po3 * po4 / ne3 + po2 * po3 * po4 * po5 / ne4;
            df[6][7] = -n[6] * po2 / ne2 - 2.0 * n[6] * po2 * po3 / ne3 - 3.0 * n[6] * po2 * po3 * po4 / ne4 - 4.0 * n[6] * po2 * po3 * po4 * po5 / ne5;
            df[7][1] = 0.0;
            df[7][2] = ph / n[7] - n[7] * phm;
            df[7][3] = phe2 / n[7] + 2.0 * phe2 * phe3 / ne2;
            df[7][4] = pc2 / n[7] + 2.0 * pc2 * pc3 / ne2 + 3.0 * pc2 * pc3 * pc4 / ne3 + 4.0 * pc2 * pc3 * pc4 * pc5 / ne4;
            df[7][5] = pn2 / n[7] + 2.0 * pn2 * pn3 / ne2 + 3.0 * pn2 * pn3 * pn4 / ne3 + 4.0 * pn2 * pn3 * pn4 * pn5 / ne4;
            df[7][6] = po2 / n[7] + 2.0 * po2 * po3 / ne2 + 3.0 * po2 * po3 * po4 / ne3 + 4.0 * po2 * po3 * po4 * po5 / ne4;

            df[7][7] = -n[2] * ph / ne2 - n[3] * phe2 / ne2 -
                4.0 * n[3] * phe2 * phe3 / ne3 -
                n[4] * pc2 / ne2 - 4.0 * n[4] * pc2 * pc3 / ne3 -
                9.0 * n[4] * pc2 * pc3 * pc4 / ne4 -
                16.0 * pc2 * pc3 * pc4 * pc5 / ne5 -
                n[5] * pn2 / ne2 - 4.0 * n[5] * pn2 * pn3 / ne3 -
                9.0 * n[4] * pn2 * pn3 * pn4 / ne4 -
                16.0 * pn2 * pn3 * pn4 * pn5 / ne5 -
                n[6] * po2 / ne2 - 4.0 * n[6] * po2 * po3 / ne3 -
                9.0 * n[4] * po2 * po3 * po4 / ne4 -
                16.0 * po2 * po3 * po4 * po5 / ne5 -
                nnaI * pna2 / ne2 - 4.0 * nnaI * pna2 * pna3 / ne3 -
                9.0 * nnaI * pna2 * pna3 * pna4 / ne4 -
                16.0 * nnaI * pna2 * pna3 * pna4 * pna5 / ne5 -
                nmgI * pmg2 / ne2 - 4.0 * nmgI * pmg2 * pmg3 / ne3 -
                9.0 * nmgI * pmg2 * pmg3 * pmg4 / ne4 -
                16.0 * nmgI * pmg2 * pmg3 * pmg4 * pmg5 / ne5 -
                nalI * pal2 / ne2 - 4.0 * nalI * pal2 * pal3 / ne3 -
                9.0 * nalI * pal2 * pal3 * pal4 / ne4 -
                16.0 * nalI * pal2 * pal3 * pal4 * pal5 / ne5 -
                nsiI * psi2 / ne2 - 4.0 * nsiI * psi2 * psi3 / ne3 -
                9.0 * nsiI * psi2 * psi3 * psi4 / ne4 -
                16.0 * nsiI * psi2 * psi3 * psi4 * psi5 / ne5 -
                nsI * ps2 / ne2 - 4.0 * nsI * ps2 * ps3 / ne3 -
                9.0 * nsI * ps2 * ps3 * ps4 / ne4 -
                16.0 * nsI * ps2 * ps3 * ps4 * ps5 / ne5 -
                nkI * pk2 / ne2 - 4.0 * nkI * pk2 * pk3 / ne3 -
                9.0 * nkI * pk2 * pk3 * pk4 / ne4 -
                16.0 * nkI * pk2 * pk3 * pk4 * pk5 / ne5 -
                ncaI * pca2 / ne2 - 4.0 * ncaI * pca2 * pca3 / ne3 -
                9.0 * ncaI * pca2 * pca3 * pca4 / ne4 -
                16.0 * ncaI * pca2 * pca3 * pca4 * pca5 / ne5 -
                nfeI * pfe2 / ne2 - 4.0 * nfeI * pfe2 * pfe3 / ne3 - 9.0 * nfeI * pfe2 * pfe3 * pfe4 / ne4 - 16.0 * nfeI * pfe2 * pfe3 * pfe4 * pfe5 / ne5 - n[2] * phm - 1.0;
        ...
        model->NH2O[i] = 0.0;
        model->rho[i] =
            1.66054e-24 * (1.008 * ah + 4.003 * ahe + 12.01 * ac +
                           14.01 * an + 16.00 * ao + 23.00 * ana + 24.32 * amg + 26.97 * aal + 28.06 * asi + 39.10 * ak + 40.08 * aca + 55.85 * afe) * model->NA[i];
        model->nmax[i] = (int)ceil(2.324 * pow(model->T[i] / (kerg * model->Ne[i]), 0.25));
...
atmosphere *model;
{
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        fprintf(out, "%2d %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e\n",
                i + 1, model->Ne[i], model->NHI[i],
                model->NHI[i] * ph / model->Ne[i], model->NHminus[i],
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        po4 = 4.830e+15 * T32 * (partfn(8.3, T, Ne) / partfn(8.2, T, Ne)) * exp(-54.9355 / kT);
        fprintf(out,
                "%2d %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e\n",
                i + 1, model->NCI[i], model->NCI[i] * pc2 / Ne,
                model->NCI[i] * pc2 * pc3 / ne2,
                model->NCI[i] * pc2 * pc3 * pc4 / ne3, model->NNI[i],
                model->NNI[i] * pn2 / Ne,
                model->NNI[i] * pn2 * pn3 / ne2,
                model->NNI[i] * pn2 * pn3 * pn4 / ne3, model->NOI[i], model->NOI[i] * po2 / Ne, model->NOI[i] * po2 * po3 / ne2, model->NOI[i] * po2 * po3 * po4 / ne3);
    }
    ...
    for (i = 0; i < Ntau; i++) {
        ...
        pti3 = 4.830e+15 * T32 * (partfn(22.2, T, Ne) / partfn(22.1, T, Ne)) * exp(-13.63 / kT);
        fprintf(out, "%2d %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e %9.3e\n",
                i + 1, model->NFeI[i], model->NFeI[i] * pfe2 / Ne,
                model->NFeI[i] * pfe2 * pfe3 / ne2, model->NTiI[i], model->NTiI[i] * pti2 / Ne, model->NTiI[i] * pti2 * pti3 / ne2, model->N3[i]);
    }

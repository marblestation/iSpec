    ...
    for (k = 0; k < nlist; k++) {
        ...
        if ((wave >= list[k].wavel && wave <= list[k].waveh) || k == 0) {
            ...
            if (flagI == 1 && code < 100.0) {
                relabund = 1.0;
                getisotope(isotope, code, line[*nline].iso, &line[*nline].atomass, &relabund);
            ...
            if (icode == 0)
                line[*nline].chi = atom[i].I1;
            else if (icode == 1)
                line[*nline].chi = atom[i].I2;
            else if (icode == 2)
                line[*nline].chi = atom[i].I3;
            else if (icode == 3)
                line[*nline].chi = atom[i].I4;
            else
                line[*nline].chi = atom[i].I2;
            line[*nline].abund = atom[i].abund;
            line[*nline].flag = 0;
            for (j = 0; j < Ntau; j++)
                line[*nline].xnum[j] = line[*nline].a[j] = line[*nline].dopp[j] = 0.0;
            list[k].flag = 1;

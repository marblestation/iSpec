...
int j;
{
    static float crossch[105][15] = { {-38.000, -38.000, -38.000, -38.000, -38.000, -38.000, -38.000,
                                       -38.000,
                                       -38.000,
    ...
    /* this factor (fac = 2.0) increases the CH continuous opacity in the
       ...
       arbitrarily, for the missing violet opacity in K-stars.  This factor is 
       changed from 1.0 to 2.0 only when the "k" flag is used on the command
       line with SPECTRUM.  On balance, it appears that the energy
       distribution of K-dwarfs is somewhat better represented WITHOUT this
       additional factor, hence the use of the "k" flag is not recommended. */
    chop = 0.0;
    ...
    chop = exp((crosscht[it - 1] + (crosscht[it] - crosscht[it - 1]) * (model->T[j] - TN) / 500.0) * 2.30258509299405);
    return (5.0 * fac * chop * model->stim[j]);
    /* Note: the factor of 5.0 here is due to the multiplicity of the ground
       state of the CH molecule.  This is introduced here, because the

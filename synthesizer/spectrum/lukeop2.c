...
int j;
{
    ...
    if (nu >= 3.517915e+15)
        x853 = seaton(nu, 3.517915e+15, 1.142e-17, 2.0, 4.29);
    if (nu >= 2.941534e+15)
        x1020 = seaton(nu, 2.941534e+15, 4.41e-18, 1.5, 3.85);
    if (nu >= 2.653317e+15)
        x1130 = seaton(nu, 2.653317e+15, 4.2e-18, 1.5, 4.34);
...
int j;
{
    ...
    if (nu >= 3.28805e+15)
        x911 = seaton(nu, 3.28805e+15, 2.94e-18, 1.0, 2.66);
    return (9.0 * x911 / u);
}
...
int j;
{
    ...
    x1169 = x824 = 0.0;
    if (nu >= 3.635492e+15)
        x824 = seaton(nu, 3.635492e+15, 1.40e-19, 4.0, 6.7);
    if (nu >= 2.564306e+15)
        x1169 = 5.11e-19 * pow(2.564306e+15 / nu, 3.0);
...
int j;
{
    ...
    if (nu >= 2.870454e+15)
        x1044 = 5.4e-20 * pow(2.870454e+15 / nu, 3.0);
    if (nu >= 2.460127e+15)
        x1218 = 1.64e-17 * sqrt(2.460127e+15 / nu);
    if (nu >= 2.110779e+15)
        x1420 = seaton(nu, 2.110779e+15, 4.13e-18, 3.0, 0.69);
...
int j;
{
    static float peach[14][6] = { {-43.8941, -43.8941, -43.8941, -43.8941, -43.8941, -43.8941},
    ...
    d1 = 1.0 - d;
    for (it = 0; it < 6; it++)
        x[it] = peach[n][it] * d + peach[n - 1][it] * d1;
    n = nt;
...
int j;
{
    return ((n1op(nu, model, j) * model->NNI[j] +
             o1op(nu, model, j) * model->NOI[j] + mg2op(nu, model,
                                                        j) * model->NMgII[j] + si2op(nu, model, j) * model->NSiII[j] + ca2op(nu, model, j) * model->NCaII[j]) * model->stim[j]);
}
